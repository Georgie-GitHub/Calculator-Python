...
class Calculator:
    def __init__(self,master):
        ...
        master.geometry('446x420+0+0')
        master.config(bg = 'gray')
        master.resizable(True,True)

        self.equation = StringVar()
        self.entry_value=''
        Entry(width=21, bg='#ccddff',font=('Arial Bold',28),textvariable=self.equation).place(x=0,y=0)

        Button(width=11,height=4,text='back',relief="flat",bg="white",command=self.backspace).place(x=360,y=50)
    ...
    def backspace(self):
        self.entry_value=str(self.entry_value[:-1])
        self.equation.set(self.entry_value)
